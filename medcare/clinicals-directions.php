<?php 
/*Template Name: Clinical Directions*/
get_header();?>
    ...
    <div class="container content">
        <div class="posts">
			...
			<?php
	$args = array(
			'post_type' => 'clinicals-directions',
			'order' => 'ASC',
			'posts_per_page' => -1,
	);
	...
	foreach( $posts->posts as $post ){
		if( ++$n % 2 != 0 ){
					...
					$test_left .= '<div class="col-md-4 col-sm-4">';
					$test_left .= '<div class="post">';
					$test_left .= '<a href="' . get_permalink() . '">';
					$test_left .= get_the_post_thumbnail( $post->ID, 'full' );
					$test_left .= '</a>';
					$test_left .= '<h3 class="post__title"><a href="' . get_permalink() . '">';
					$test_left .= get_the_title();
					$test_left .= '</a></h3>';
					//$test_left .= '<p>'.($post->post_content, 100).'</p>';
					$test_left .= '<p>'.substr(strip_tags($post->post_excerpt), 0, 350).'</p>';
					$test_left .= '<a class="post__btn" href="' . get_permalink() . '">Подробнее</a>';
					$test_left .= '</div>';
					$test_left .= '</div>';
		}else{
					$test_right .= '<div class="col-md-4 col-sm-4">';
					$test_right .= '<div class="post">';
					$test_right .= '<a href="' . get_permalink() . '">';
					$test_right .= get_the_post_thumbnail( $post->ID, 'full' );
					$test_right .= '</a>';
					$test_right .= '<h3 class="post__title"><a href="' . get_permalink() . '">';
					$test_right .= get_the_title();
					$test_right .= '</a></h3>';
					//$test_right .= '<p>'.($post->post_content, 100).'</p>';
					$test_right .= '<p>'.substr(strip_tags($post->post_excerpt), 0, 350).'</p>';
